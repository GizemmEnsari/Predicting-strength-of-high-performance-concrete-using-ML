...
param_grid = {'alpha': np.logspace(-4, 4, 9)} #1/10^4 to 10^4
...
# Train the final Ridge regression model with the best hyperparameters on the entire training dataset
ridge_final = Ridge(alpha=grid_search.best_params_['alpha']).fit(X_train, y_train)

# Evaluate the final model on the test dataset
X_test = test_data.iloc[:, :-1].values
y_test = test_data.iloc[:, -1].values

Err_test = np.mean((ridge_final.predict(X_test) - y_test) ** 2)
print(f"Err using Ridge regression: {Err_test:.5f}")
...
plt.scatter(alphas, neg_mse, marker='x', s=200, linewidths=3, color='r')
plt.title('Ridge regression performance as a function of alpha')
plt.show()
